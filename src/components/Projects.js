        ...
        >
          ...
          <Stack px={4} spacing={4}>
            {projects.map((project) => (
              <Fade bottom key={JSON.stringify(project)}>
                <Card
                  key={project.name}
                  direction={{
                    base: "column",
                ...
                >
                  ...
                  <Stack>
                    <CardBody align="left">
                      <Heading size="md">{HeadingFormat(project.name)}</Heading>
          ...
          <Center px={4}>
            <ButtonGroup variant="outline">
              ...
              >
                All {selected === "All" ? `[${others.length}]` : ""}
              </Button>
              {options.map((option,i) => (
                ...
                >
                  {HeadingFormat(option.value)} {selected === option.value ? `[${others.filter((other) => {
                if (selected === "All") {
                  return true;
                } else {
          ...
          <SimpleGrid columns={[1, 2, 3]} px={4} spacing={4}>
            {others
              ...
              .map((other) => (
                <Fade bottom key={JSON.stringify(other)}>
                  <Card key={other.name}>
                    <Stack>
                      <CardBody align="left" h={[null, "40vh"]}>
                        <Heading size="sm">{HeadingFormat(other.name)}</Heading>
